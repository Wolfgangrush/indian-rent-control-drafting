# 🇮🇳 Indian Rent Control Drafting — Gemini CLI Port

> The same `indian-rent-control-drafting` pipeline you know, now invokable from Gemini CLI. **Same 6-agent pipeline. Same canonical agent specs. Same filing-grade output. Different LLM brain.**
...
├── skills/ (state exemplars + case-type templates)                   ← case-type templates (unchanged)
...
```bash
cd "indian-rent-control-drafting"
gemini
```
...
**Forum:** Rent Controller / Court of Small Causes / Rent Authority / Rent Tribunal / Prescribed Authority / Civil Judge as Rent Controller — per State Act
