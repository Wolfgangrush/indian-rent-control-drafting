# GEMINI.md — Indian Rent Control Drafting (Gemini CLI port)
...
**Plugin:** `indian-rent-control-drafting` · **Forum:** Rent Controller / Court of Small Causes / Rent Authority / Rent Tribunal / Prescribed Authority / Civil Judge as Rent Controller — per State Act · **Publisher:** wolfgang_rush · **License:** MIT

---

## 🇮🇳 WHO YOU ARE TALKING TO

The user is an **enrolled Indian advocate** drafting rent-control pleadings for filing. They are running the 6-agent drafting pipeline (reader → format → drafter → verifier → refiner → overseer) and expect filing-grade output (.docx) with anti-hallucination discipline.
...
Every pipeline run reads the user's `case-config.md + state-config.md` from the case folder. Substitute every `{{bench_config.X}}` / `{{case_config.X}}` placeholder in the format-shell with the real value before passing to the Drafter.
...
- Preserve the advocate's bench's idiom strictly (substituted from case-config.md + state-config.md)
...
Eviction (bona-fide need · sub-letting · arrears · damaging) · Standard-rent fixation · Rent-deposit · Section 14(1)(e) DRC Act summary · Statutory tenancy heritability (Damadilal · Gian Devi Anand) · Pagdi (Section 56 MRC Act)

Each case type has its own template in `skills/ (state exemplars + case-type templates)`. The Format agent loads the matching one based on the user's request.
